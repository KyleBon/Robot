...
class SimplePickAndPlaceExample():
	...
	def pick_up_object_near_point(self, target_point, whicharm):
		...
		self.papm.point_head(get_xyz(target_point.point), target_point.header.frame_id)
		
		rospy.loginfo("detecting the table and objects")
		self.papm.call_tabletop_detection(update_table = 1, update_place_rectangle = 1, clear_attached_objects = 1)
		...
		if success:
			rospy.loginfo("pick-up was successful! Moving arm to the side")
	...
	def place_object(self, whicharm, place_rect_dims, place_rect_center):
		...
		rospy.loginfo("putting down the object in the %s gripper"\%self.papm.arm_dict[whicharm])
		
		success = self.papm.put_down_object (whicharm, max_place_tries = 25, use_place_override = 1)
...
if __name__ == "__main__":
	...
	#.5 m in front of the robot, centered
	...
	if success:
		...
		center_quat = [0,0,0,1]
		place_rect_center = create_pose_stamped(center_xyz + center_quat, 'base_link')
